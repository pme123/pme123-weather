...
object ForecastGraph:
  ...
  def apply(
    ...
    def getColor(windType: WindType): String = windType match {
      case WindType.FöhnSehrStark => "rgba(183, 28, 28, 0.85)"    // Very Dark Red (storm)
      case WindType.FöhnStark => "rgba(211, 47, 47, 0.85)"        // Dark Red (strong)
      case WindType.FöhnGut => "rgba(239, 83, 80, 0.8)"           // Medium Red (good)
    ...
    val layout = Layout()
      .withFont(Font().withColor(Color.StringColor("rgba(241, 245, 249, 0.9)")))
      .withXaxis(Axis()
        .withTitle("Zeit")
        .withTickangle(-45)
        .withTicktext(tickLabels)
        .withTickvals(displayTimes)
        .withGridcolor(Color.StringColor("rgba(255, 255, 255, 0.12)"))
        .withShowgrid(true))
      .withYaxis(Axis()
      ...
      .withShowlegend(true)
      .withAutosize(true)
      .withMargin(Margin().withL(60).withR(30).withT(20).withB(120))
      .withLegend(Legend()
        .withX(0.5)
        .withY(-0.25)
        .withBgcolor(Color.StringColor("rgba(30, 41, 59, 0.95)"))
        .withBordercolor(Color.StringColor("rgba(71, 85, 105, 0.8)"))
        .withOrientation(Orientation.Horizontal)
        .withXanchor(Anchor.Center)
        .withYanchor(Anchor.Top)
        .withFont(Font()
          .withSize(13)
          .withColor(Color.StringColor("rgba(241, 245, 249, 1.0)"))))
