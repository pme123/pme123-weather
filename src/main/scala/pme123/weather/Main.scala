...
import pme123.weather.config.{ConfigStore, WeatherConfig}
...
object Main:
  ...
  def main(args: Array[String] = Array.empty): Unit =
    ...
    // customConfigsVar/activeConfigVar are already populated synchronously from
    // localStorage, so the app can render immediately - folder sync is a background
    // enhancement that updates those Vars (and so the UI) reactively once it resolves.
    ConfigStore.initFolderSync()
  ...
  private lazy val page =
    div(
      ...
      div(
        ...
        // Rebuild the tabs/view whenever another configuration is activated - no page
        // reload needed. `stationDiffs` is reassigned right before the rebuild so every
        // existing consumer (WeatherTabs, WeatherView, MapView, ...) sees the new data.
        child <-- ConfigStore.activeConfigVar.signal.map: config =>
          stationDiffs = WeatherConfig.fromConfig(config.visible)
          val selectedTabVar = Var(MapView.tabId)
          div(
